import {fetchCurrentWeather, fetchHistoricalWeather} from "./Services/WeatherStack";
...
class Controller {
    constructor(model, view) {
        ...
        this.view.bindOnLocationInputSelect(this.onLocationInputSelect.bind(this));
        this.view.bindOnLocationInputChange(this.onLocationInputChange.bind(this));
        this.view.bindOnCheckboxStateChange(this.onCheckboxStateChange.bind(this));
    ...
    async onLocationInputSelect(location) {
        ...
        if (this.isHistoricalSearchEnabled) {
            ...
            const { current, historical } = await fetchHistoricalWeather({ location, date: this.historicalDate });
    ...
    async onDateSelect(date) {
        if (date && location) {
            ...
            this.view.renderHistoricalWeatherLoader();
            const { historical } = await this.model.fetchHistoricalWeather(date);
            this.view.renderHistoricalWeather(historical);
        }
    }
}
